...
function useThemeIcon() {
  ...
  return mounted && resolvedTheme === "light" ? iconLight : iconDark;
}
...
function ChatSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-[17rem] flex-col bg-card transition-all md:relative md:z-auto md:translate-x-0",
      ...
      >
        <div className="flex h-14 items-center justify-between px-3">
          ...
          <button
            ...
            className="inline-flex size-8 items-center justify-center rounded-full text-foreground/55 hover:bg-foreground/10"
        ...
        <nav className="space-y-1 px-3">
          ...
          <SidebarAction
            ...
            collapsed={isCollapsed}
          />
          <label className={cn("flex h-11 w-full items-center gap-3 rounded-2xl px-3 text-sm font-medium text-foreground/75 transition focus-within:bg-card", isCollapsed && "justify-center")}>
        ...
        {!isCollapsed && <div className="mt-6 px-4 text-xs font-medium text-foreground/45">Test history</div>}
        <div className="mt-2 min-h-0 flex-1 overflow-y-auto px-2">
          {isCollapsed ? null : conversations.length === 0 ? (
            <div className="px-3 text-sm text-foreground/35">No tests yet.</div>
          ) : (
            conversations.map((item) => (
              ...
              >
                <MessageSquare className="mt-0.5 size-3.5 shrink-0 text-foreground/45" />
                <span className="min-w-0">
                  <span className="block truncate">{conversationTitle(item)}</span>
                  <span className="mt-0.5 block text-[11px] text-foreground/35">
                  ...
                  {(item.labels ?? []).length > 0 && (
                    <span className="mt-2 flex flex-wrap gap-1">
                      {(item.labels ?? []).slice(0, 2).map((label) => (
                        <span
                          key={label}
                          className="rounded-full bg-foreground/8 px-2 py-0.5 text-[10px] text-foreground/45"
                        >
                          {label}
                        </span>
                      ))}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-card text-foreground" : "text-foreground/75 hover:bg-foreground/8",
      )}
    >
    ...
    <header className="relative z-20 flex h-[3.25rem] items-center justify-between border-b border-border/[0.04] px-3 md:px-5">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          type="button"
          onClick={onOpenInspector}
          className="inline-flex size-9 items-center justify-center rounded-full text-foreground/65 hover:bg-foreground/10"
  ...
  return (
    <div className={cn("relative", className)}>
      <button
        ...
        className="flex h-9 max-w-full items-center gap-2 rounded-full border border-border bg-card/95 px-3 text-left text-xs font-medium text-foreground shadow-lg shadow-black/10 transition hover:border-border hover:bg-card focus:border-primary/70 focus:outline-none"
      >
        <span className="min-w-0 truncate">{selected?.label}</span>
        <ChevronDown className={cn("size-4 shrink-0 text-foreground/55 transition", open && "rotate-180")} />
      </button>
      {open && (
        <div className="jaabili-pop-in absolute left-0 top-10 z-50 max-h-72 min-w-60 overflow-y-auto rounded-2xl border border-border bg-card p-1 shadow-2xl shadow-black/40">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
            ...
            >
              <span className="min-w-0">
                ...
                {option.description && (
                  <span className="mt-0.5 block truncate text-xs text-foreground/35">
                    {option.description}
                  </span>
                )}
              </span>
              {option.disabled ? (
                <span className="shrink-0 rounded-full bg-foreground/8 px-2 py-0.5 text-[10px] font-semibold text-foreground/40">
                  Soon
                </span>
              ) : (
                option.value === value && (
...
function VisitorSimulatorPanel({
  ...
  return (
    <section className="jaabili-rise-in mx-auto w-full max-w-5xl rounded-3xl border border-border bg-card/75 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-sm font-semibold text-foreground">Visitor simulator</div>
        ...
        <button
          ...
          onClick={onToggle}
          disabled={!canTest}
          className="rounded-full border border-border bg-foreground/[0.04] px-4 py-2 text-sm font-medium text-foreground/75 transition hover:bg-foreground/8 disabled:cursor-not-allowed disabled:opacity-45"
        >
...
function ConversationOutcomeStrip({
  ...
  return (
    <div className="mt-2 grid gap-2 border-b border-border pb-3 sm:grid-cols-4">
      <OutcomePill
        label="Lead status"
...
function OutcomePill({
  ...
  return (
    ...
    >
      ...
      <div className="mt-0.5 truncate text-xs font-medium text-foreground/75">{value}</div>
    </div>
  );
}
        ...
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <button
              ...
              className={cn(
                "inline-flex size-9 shrink-0 items-center justify-center rounded-full text-foreground/80 transition hover:bg-foreground/10",
                isAttachOpen && "bg-foreground/10 text-foreground",
              )}
          ...
          <textarea
            ...
            className="max-h-32 min-h-10 flex-1 resize-none bg-transparent py-2.5 text-[15px] leading-6 text-foreground outline-none placeholder:text-foreground/45"
          />
...
function AttachmentMenu({
  ...
  return (
    <div className="jaabili-pop-in fixed bottom-32 left-4 right-4 z-40 overflow-hidden rounded-[1.75rem] border border-border bg-card p-2 shadow-2xl shadow-black/40 sm:absolute sm:bottom-14 sm:left-0 sm:right-auto sm:w-64">
      <AttachItem
        icon={Upload}
        label="Upload"
        onClick={onUpload}
...
function AttachItem({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-12 w-full items-center gap-3 rounded-2xl px-3 text-left transition hover:bg-foreground/8"
    >
      <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-foreground/8 text-foreground/80">
        <Icon className="size-5" />
      </span>
...
function KnowledgeDialog({
  ...
  return (
    <>
      ...
      />
      <div className="fixed left-1/2 top-1/2 z-[90] w-[34rem] max-w-[92vw] -translate-x-1/2 -translate-y-1/2 rounded-[2rem] border border-border bg-card p-5 shadow-2xl shadow-black/50">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold">{labels.heading}</h2>
            <p className="mt-1 text-sm leading-6 text-foreground/50">{labels.body}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <label className="mb-4 block text-sm text-foreground/55">
          Title
          <input
            value={title}
        ...
        <label className="block text-sm text-foreground/55">
          ...
          {isWebsite ? (
            <input
              ...
              className="mt-2 h-12 w-full rounded-2xl border border-border bg-card px-4 text-foreground outline-none placeholder:text-foreground/30 focus:border-primary/50"
            />
          ) : (
            <textarea
              value={body}
              ...
              placeholder={labels.placeholder}
              rows={8}
              className="mt-2 w-full resize-none rounded-2xl border border-border bg-card px-4 py-3 text-foreground outline-none placeholder:text-foreground/30 focus:border-primary/50"
            />
          )}
        </label>
        ...
        <div className="mt-5 flex justify-end gap-3">
          <button
            ...
            className="h-11 rounded-full px-5 text-sm font-medium text-foreground/70 hover:bg-foreground/8"
          >
            Cancel
          </button>
          <button
            ...
            onClick={submit}
            disabled={!canSubmit}
            className="inline-flex h-11 min-w-32 items-center justify-center gap-2 rounded-full bg-foreground px-5 text-sm font-medium text-background disabled:cursor-not-allowed disabled:opacity-40"
          >
    ...
    <div className={cn("jaabili-message-in flex gap-3", isVisitor && "justify-end")}>
      ...
      <div
        className={cn(
          ...
          isVisitor ? "bg-card text-foreground" : "text-foreground/78",
        )}
      >
        {!isVisitor && animate ? (
...
function InspectorDrawer({
  ...
  return (
    <>
      ...
      />
      <aside
        className="fixed inset-y-0 right-0 z-50 w-[23rem] max-w-[92vw] overflow-y-auto bg-card p-5 shadow-2xl shadow-black/40"
      >
        ...
        <InspectorBlock title="Knowledge">
          <div className="mb-2 rounded-2xl bg-card p-4">
          ...
          {tenantSources.length > 0 && (
            <div className="mt-2 space-y-2">
              {tenantSources.slice(0, 4).map((source) => (
                <div
                  key={source.id}
                  className="rounded-2xl bg-card px-4 py-3 text-sm"
                >
                  <div className="truncate text-foreground/80">{source.title}</div>
                  <div className="mt-1 truncate text-xs text-foreground/35">
            ...
            <div className="space-y-3">
              <div className="rounded-2xl border border-primary/20 bg-card p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="rounded-full bg-foreground/10 px-2 py-1 text-xs text-primary">
              ...
              <div className="space-y-2">
                {operationsReport.actionQueue.slice(0, 4).map((action) => (
                  <div key={action.id} className="rounded-2xl bg-card p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-medium text-foreground/85">
                          {action.title}
          ...
          {conversationAudit ? (
            <div className="space-y-3">
              ...
              {conversationAudit.risks.slice(0, 3).map((risk) => (
                <div key={risk} className="rounded-xl bg-card px-3 py-2 text-xs leading-5 text-foreground/55">
                  {risk}
                </div>
              ))}
              {conversationAudit.missingLeadFields.length > 0 && (
                <div className="rounded-xl bg-card px-3 py-2 text-xs leading-5 text-foreground/45">
        ...
        <InspectorBlock title="Notifications">
          ...
          <button
            type="button"
            onClick={onSave}
            className="mt-3 h-10 w-full rounded-full bg-foreground text-sm font-medium text-background"
          >
            Save
          </button>
        </InspectorBlock>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-2xl border border-border bg-foreground/[0.03] p-3">
          <p className="text-foreground/45">Tenant</p>
          <p className="mt-1 truncate font-semibold text-foreground">
            {tenant?.name ?? "Demo tenant"}
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-foreground/[0.03] p-3">
          <p className="text-foreground/45">Launch</p>
          <p className="mt-1 font-semibold text-emerald-200">{launchState}</p>
        </div>
        <div className="rounded-2xl border border-border bg-foreground/[0.03] p-3">
          <p className="text-foreground/45">Sources</p>
          <p className="mt-1 font-semibold text-foreground">{sourceCount}</p>
        </div>
        <div className="rounded-2xl border border-border bg-foreground/[0.03] p-3">
          <p className="text-foreground/45">Widget key</p>
      ...
      />
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-foreground/45">Paste before the closing body tag.</p>
        <button
          className="inline-flex shrink-0 items-center gap-2 rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background transition hover:opacity-90"
          type="button"
          onClick={copyScript}
        >
...
function CompanyOnboardingDrawer({
  ...
  return (
    <>
      ...
      />
      <aside className="fixed inset-y-0 right-0 z-50 flex w-[42rem] max-w-full flex-col bg-card shadow-2xl shadow-black/50">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <section className="mt-4 rounded-3xl border border-border bg-card/80 p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <div className="rounded-full bg-foreground/8 px-3 py-1 text-xs text-foreground/55">
                {sourceCount}/5 ready
              </div>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="text-xs text-foreground/38">
                        {source.complete ? "ready" : "required"}
                      </span>
                    </span>
                    {source.complete && <Check className="size-4 text-primary" />}
                  </button>
                );
              })}
            </div>
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 flex w-[28rem] max-w-[94vw] flex-col bg-card p-5 shadow-2xl shadow-black/40 md:left-[18rem]">
        ...
        <div className="my-5 h-px bg-foreground/8" />
        ...
        {learningReport && (
          <div className="mb-5 rounded-3xl border border-primary/20 bg-card p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="space-y-2">
                  {learningReport.recommendedLabels.slice(0, 3).map((item) => (
                    <div key={item.conversationId} className="text-xs leading-5">
                      ...
                      <div className="mt-1 text-foreground/38">
                        {item.labels.join(", ")}
                      </div>
                    </div>
                  ))}
          ...
          {sourceOptions.map((source) => {
            ...
            return (
              <button
                ...
                className="rounded-2xl bg-card p-4 text-left text-sm hover:bg-foreground/8"
              >
        ...
        <div className="mt-6 min-h-0 flex-1 overflow-y-auto">
          {sources.length === 0 ? (
            ...
            <div className="space-y-2">
              {sources.map((source) => (
                <div key={source.id} className="rounded-2xl bg-card p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
  ...
  return (
    <span
      className={cn(
        ...
        value === "ready" && "bg-primary/15 text-primary",
        value === "processing" && "bg-primary/15 text-foreground",
        value === "pending" && "bg-secondary/15 text-secondary",
        ...
        >
          ...
          <span
            className={cn(
              ...
              settings.strictGrounding ? "bg-primary" : "bg-foreground/15",
            )}
          >
            <span
              className={cn(
            ...
            <button
              ...
              className={cn(
                ...
                active
                  ...
                  : "border-border text-foreground/45 hover:bg-foreground/8 hover:text-foreground/75",
              )}
            >
              {label}
            </button>
...
function InspectorBlock({
  ...
  return (
    <section className="mb-6">
      <h2 className="mb-3 text-sm font-medium text-foreground/55">{title}</h2>
      {children}
    </section>
  );
}
...
function InfoRow({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="flex justify-between gap-4 rounded-2xl bg-card px-4 py-3">
  ...
  return (
    <label className="mb-3 block text-sm text-foreground/45">
      ...
      <input
        ...
        className="mt-1 h-11 w-full rounded-2xl border border-border bg-card px-4 text-foreground outline-none focus:border-primary/50"
      />
    </label>
  );
}
  ...
  return (
    <div className="mb-4 rounded-2xl border border-border bg-black/16 p-3">
      <div className="mb-3">
        ...
        <div className="mt-1 text-xs leading-5 text-foreground/36">{description}</div>
      </div>
      {children}
    </div>
  );
